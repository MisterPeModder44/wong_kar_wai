...
int				t_grid_move(t_move move, t_grid *grid)
{
	...
	{
		if (move == MOVE_DOWN)
			move_value += t_grid_calcul_column_down(grid, i);
		else if (move == MOVE_UP)
			move_value += t_grid_calcul_column_up(grid, i);
		else if (move == MOVE_LEFT)
			move_value += t_grid_calcul_line_left(grid, i);
		else if (move == MOVE_RIGHT)
			move_value += t_grid_calcul_line_right(grid, i);
		i++;
	}
	return (move_value);
}
